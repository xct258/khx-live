...
`khx-live` 是一个面向 B 站 UP 主“括弧笑”的直播录制、备份与管理容器化解决方案。它封装在一个 Debian 基础镜像内，整合了 **录播姬、biliup、DanmakuFactory** 等工具，并可选附带一个在线切片的 Web 应用，从而提供从直播获取到视频处理的全栈支持。

> 注意：本项目主要用于个人/小规模自动化需求，依赖 B 站及相关工具的接口，使用时请遵守平台规则。
...
1. **构建镜像**（如需本地修改或测试）：
   ```sh
   docker build -t xct258/khx-live .
   ```
...
- **更新录播文件目录**：修改挂载路径或在 `/rec/录播姬` 下新增子目录。
- **添加自定义脚本**：将脚本放入宿主的 `data/脚本` 目录即可在容器启动时自动复制。
- **自行替换字体**：在 `data/字体` 下放入 TTF 目录，容器启动时会同步更新。
- **备份计划**：容器内 `/usr/local/bin/执行视频备份脚本.sh` 是每日定时任务，可根据需要修改。
